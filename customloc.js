...
if (typeof(global.loc)=="undefined")
    global.loc=this;

if (typeof(global.def)=="undefined")
  global.def=new Array();
...
{
  if (dirname in global.def)
  {
    return global.def[dirname];
  }
  else
  {
	var fs = require('fs');
    ...
    global.def[dirname]={};
	// Declare variables
	global.def[dirname].m_mapDictLocal=new Array();
	global.def[dirname].m_strLocal=new Array();
	modulename=dirname.substr(pos+1, 99).toUpperCase();
	global.def[dirname].modulename=modulename;
	// Declare functions
	global.def[dirname].release=release_l;
	global.def[dirname].addLocalString=addLocalString;
	global.def[dirname].getValue=getValue;
	global.def[dirname].setValue=setValue;
	global.def[dirname].addDictEntry=addDictEntry;
	global.def[dirname].addDictGlobalEntry=addDictGlobalEntry;
	global.def[dirname].removeDictEntry=removeDictEntry;
	
	global.def[dirname].resetDict=resetDict;
	global.def[dirname].removeDictGlobalEntry=removeDictGlobalEntry;
	global.def[dirname].findLocalString=findLocalString;
	global.def[dirname].completeLocalString=completeLocalString;
	global.def[dirname].getLocalString=getLocalString;
	global.def[dirname].getLocalStringAndSplit=getLocalStringAndSplit;
	global.def[dirname].getLocalStringArray=getLocalStringArray;
	global.def[dirname].getLocalStringArraySize=getLocalStringArraySize;
	global.def[dirname].debug=debug_l;
	...
	fs.readFileSync(file).toString().split("\n").forEach(function(line)
	{
	  if (line!="")
	  {
		...
		if (v.length!=2)
		  console.log(__dirname+"\\customloc.js: Error at line "+count+" in file "+ file);
		else
		{
		  if (v[1].indexOf("\r")!=-1)
			  global.def[dirname].m_strLocal[v[0]]=v[1].substr(0,v[1].length-1);
		  else
			  global.def[dirname].m_strLocal[v[0]]=v[1];
		}
	  }
	  count++;
	});
...
{
	for (var k in global.def)
	{
		if (typeof def[k]!=="undefined")
			global.def[k].release();
	}
...
var findLocalString=function(ID)
{
    ...
    console.log(__dirname+"\\customloc.js: cannot found ID '"+ID+"'");
	return "";
}
...
function debug()
{
	for (var k in global.def)
	{
		//console.log(global.def[k]);
		console.log("********Showing " + global.def[k].modulename + "*********");
		global.def[k].debug();
	}
